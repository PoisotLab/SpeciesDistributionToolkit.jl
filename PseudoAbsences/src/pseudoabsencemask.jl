...
"""
    pseudoabsencemask(::Type{T}, presences::SDMLayer{Bool}; background::Bool=false, kwargs...) where {T<:PseudoAbsenceGenerator}

This function will generate a pseudoabsencemask for all
`PseudoAbsenceGenerator` methods. The important keyword argument is
`background`, which defaults to `false`, and regulates whether the mask
returned will be pseudo-absences (known presences are excluded) or background
points (known presences are included). Using `background=true` is likely to
only ever be useful for MaxEnt models.

All internal work is done by `PseudoAbsences.__pa_mask`, which uses different
arguments baed on the generator that was selected.
"""
function pseudoabsencemask(::Type{T}, presences::SDMLayer{Bool}; background::Bool=false, kwargs...) where {T<:PseudoAbsenceGenerator}
  potential_background = __pa_mask(T, presences; kwargs...)
  if background
    return potential_background
  else
    _zero_occ!(potential_background, presences)
    return potential_background
  end
end

"""
    __pa_mask(::Type{RandomSelection}, presences::SDMLayer{Bool})
...
layer.
"""
function __pa_mask(::Type{RandomSelection}, presences::SDMLayer{Bool})
...
"""
    __pa_mask(::Type{SurfaceRangeEnvelope}, presences::SDMLayer{Bool})
...
"""
function __pa_mask(::Type{SurfaceRangeEnvelope}, presences::SDMLayer{Bool})
  _layer_works_for_pseudoabsence(presences)
...
"""
    __pa_mask( ::Type{DistanceToEvent}, presences::SDMLayer{Bool}; f = minimum, )
...
"""
function __pa_mask(::Type{DistanceToEvent}, presences::SDMLayer{Bool}; f=minimum)
  _layer_works_for_pseudoabsence(presences)
  presence_only = nodata(presences, false)
...
"""
    __pa_mask(::Type{WithinRadius}, presences::SDMLayer{Bool}; distance::Number = 100.0, )
...
function __pa_mask(
  ...
  _layer_works_for_pseudoabsence(presences)
  bg = __pa_mask(DistanceToEvent, presences; kwargs...)
  background = bg .<= distance
  return background
end

"""
    __pa_mask(::Type{WithoutRadius}, presences::SDMLayer{Bool}; distance::Number = 100.0, )
...
"""
    __pa_mask(::Type{BetweenRadius}, presences::SDMLayer{Bool}; closer::Number = 10.0, further::Number=100.0)
...
"""
    __pa_mask( ::Type{DegreesToEvent}, presences::SDMLayer{Bool}; f = minimum, absolute::Bool = false)
...
"""
function __pa_mask(::Type{DegreesToEvent}, presences::SDMLayer{Bool}; f=minimum, absolute::Bool=false)
  _layer_works_for_pseudoabsence(presences)
  presence_only = nodata(presences, false)
  background = zeros(presences, Float64)
...
"""
    __pa_mask(::Type{WithinDegrees}, presences::SDMLayer{Bool}; distance::Number = 3.0, )

Generates a mask for pseudo-absences where pseudo-absences can be within a
`distance` (in degrees) of the original observation. Internally, this uses
`DegreesToEvent`. All other keyword arguments are passed to `__pa_mask`.
"""
function __pa_mask(
  ::Type{WithinDegrees},
  presences::SDMLayer{Bool};
  ...
  _layer_works_for_pseudoabsence(presences)
  bg = __pa_mask(DegreesToEvent, presences; kwargs...)
  background = bg .<= distance
  return background
end

"""
    __pa_mask(::Type{WithoutDegrees}, presences::SDMLayer{Bool}; distance::Number = 3.0, )
...
"""
    __pa_mask(::Type{BetweenDegrees}, presences::SDMLayer{Bool}; closer::Number = 1.0, further::Number=3.0)
