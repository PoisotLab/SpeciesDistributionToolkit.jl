...
"""
    pseudoabsencemask(::Type{RandomSelection}, presences::SDMLayer{Bool}; background::Bool=false)
...
The `background` keyword (defaults to `false`) is used to determine whether the
presence locations can be part of the background point data.
"""
function pseudoabsencemask(::Type{RandomSelection}, presences::SDMLayer{Bool}; background::Bool=false)
...
@testitem "We can use RandomSelection with background points" begin
  using PseudoAbsences.SimpleSDMLayers
  n = SDMLayer(zeros(Bool, 10, 10))
  n.grid[1] = true
  n.grid[end] = true
  p = pseudoabsencemask(RandomSelection, n; background=true)
  @test all(p.grid)
end

"""
    pseudoabsencemask(::Type{SurfaceRangeEnvelope}, presences::SDMLayer{Bool}; background::Bool=false)
...
presence locations can be part of the background point data.
"""
function pseudoabsencemask(::Type{SurfaceRangeEnvelope}, presences::SDMLayer{Bool}; background::Bool=false)
  _layer_works_for_pseudoabsence(presences)
  presence_only = nodata(presences, false)
  bgarea = zeros(presences, Bool)
...
@testitem "We can use SurfaceRangeEnvelope with background" begin
  using PseudoAbsences.SimpleSDMLayers
  n = SDMLayer(zeros(Bool, 10, 10))
  n.grid[2, 2] = true
  n.grid[9, 9] = true
  p = pseudoabsencemask(SurfaceRangeEnvelope, n; background=true)
  for i in axes(n.grid, 1)
    for j in axes(n.grid, 2)
      if (2 <= i <= 9) & (2 <= j <= 9)
        # Test the surface range
        @test p.grid[i, j] == true
      else
        # Test the exterior borders (all false)
        @test p.grid[i, j] == false
      end
...
"""
    pseudoabsencemask(::Type{DistanceToEvent}, presences::SDMLayer{Bool}; f=minimum, background::Bool=true)

Generates a mask for pseudo-absences using the distance to event method.
Candidate cells are weighted according to their distance to a known
observation, with far away places being more likely. Depending on the
distribution of distances, it may be a very good idea to flatten this layer
using `log` or an exponent. The `f` function is used to determine which
distance is reported (`minimum` by default, but can be any other relevant
function).

The `background` keyword (defaults to `false`) is used to determine whether the
presence locations can be part of the background point data in all other
functions, but is a noop here. Setting it to either value makes no difference
as the distance to an event will always be represented as 0.
"""
function pseudoabsencemask(::Type{DistanceToEvent}, presences::SDMLayer{Bool}; f=minimum, background::Bool=true)
  _layer_works_for_pseudoabsence(presences)
  presence_only = nodata(presences, false)
  bgarea = zeros(presences, Float64)
...
"""
    pseudoabsencemask( ::Type{WithinRadius}, presences::SDMLayer{Bool}; distance::Number=100.0, background::Bool=false, kwargs...)
...
function pseudoabsencemask(
  ...
  if background
    ...
    return _zero_occ!(bgarea, presences)
  end
end

"""
    pseudoabsencemask( ::Type{WithoutRadius}, presences::SDMLayer{Bool}; distance::Number=100.0, background::Bool=false, kwargs...)
...
"""
    pseudoabsencemask( ::Type{BetweenRadius}, presences::SDMLayer{Bool}; closer::Number=10.0, further::Number=100.0, background::Bool=false, kwargs...)
