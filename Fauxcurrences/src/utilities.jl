...
function _generate_new_random_point(layer, points, distances)
    point =
        _random_point(points[:, rand(1:size(points, 2))], sqrt(rand()) * rand(distances))
    while isnothing(layer[point...])
        point = _random_point(
            points[:, rand(1:size(points, 2))],
            sqrt(rand()) * rand(distances),
        )
    end
    return point
end
...
function _bin_distribution(D::Matrix{Float64}, m::Float64)::Vector{Float64}
    ...
    return _bin_distribution!(c, D, m)
end
...
function preallocate_distance_matrices(obs; samples = size.(obs, 2))
    ...
    obs_inter = [
        zeros(Float64, (size(obs[i], 2), size(obs[j], 2))) for i in 1:(length(obs) - 1)
        for j in (i + 1):length(obs)
    ]
    sim_intra = [zeros(Float64, (samples[i], samples[i])) for i in 1:length(obs)]
    sim_inter = [
        zeros(Float64, (samples[i], samples[j])) for i in 1:(length(obs) - 1) for
        j in (i + 1):length(obs)
    ]
    ...
    for i in 1:(length(obs) - 1)
        for j in (i + 1):length(obs)
            if (i in updated) | (j in updated)
                Distances.pairwise!(
                    inter[cursor],
                    Fauxcurrences._distancefunction,
                    obs[i],
                    obs[j],
                )
            end
...
function score_distributions(W, bin_intra, bin_s_intra, bin_inter, bin_s_inter)
    ...
    for i in eachindex(bin_intra)
        for j in 1:length(bin_intra)
            if j >= i
                if i == j
                    M[i, i] = Fauxcurrences._distance_between_binned_distributions(
                        bin_intra[i],
                        bin_s_intra[i],
                    )
                else
                    M[i, j] =
                        M[j, i] = Fauxcurrences._distance_between_binned_distributions(
                            bin_inter[cursor],
                            bin_s_inter[cursor],
                        )
                    cursor += 1
