...
@testitem "We can get all Resolve Ecoregions" begin
    prov = PolygonData(Resolve, Ecoregions)
    polys = getpolygon(prov)
    @test polys isa FeatureCollection
end

@testitem "We can access Resolve by Realm" begin
    prov = PolygonData(Resolve, Ecoregions)
    polys = getpolygon(prov)
    byrealm = polys["Realm"=>"Australasia"]
    @test byrealm isa FeatureCollection
end

@testitem "We can access Resolve by name" begin
    prov = PolygonData(Resolve, Ecoregions)
    polys = getpolygon(prov)
    byname = polys["Adelie Land tundra"]
    @test byname isa Feature
end
