...
function Base.intersect(fc1::FeatureCollection, f1::Feature)
    ints = filter(!isnothing, [intersect(f, f1) for f in fc1])
    feats = [Feature(int, f1.properties) for int in ints]
    return FeatureCollection(feats)
end
...
function Base.intersect(p1::POLY_AND_MP, p2::POLY_AND_MP)
    GI.crs(p1.geometry) == GI.crs(p2.geometry) ||
        throw(ArgumentError("Cannot intersect two geometries with different CRSs"))
    p = AG.intersection(p1.geometry, p2.geometry)
    _add_crs(p, AG.importCRS(GI.crs(p1.geometry)))
    if p isa AG.IGeometry{AG.wkbGeometryCollection}
        n = AG.ngeom(p)
        for i in Base.OneTo(n)
            if AG.getgeom(p, i) isa AG.IGeometry{AG.wkbLineString}
                return nothing
            else
                k = AG.getgeom(p, i)
                return k isa AG.IGeometry{AG.wkbPolygon} ? Polygon(k) : MultiPolygon(k)
            end
        end
    end
    if p isa AG.IGeometry{AG.wkbPolygon}
        return Polygon(p)
    end
    if p isa AG.IGeometry{AG.wkbMultiPolygon}
