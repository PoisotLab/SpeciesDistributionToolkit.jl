...
Base.CartesianIndices(layer::SDMLayer) = CartesianIndices(layer.grid)[findall(!isequal(layer.nodata), layer.grid)]
Base.length(layer::SDMLayer) = count(!isequal(layer.nodata), layer.grid)
